...
Proof, not paperwork — Annex reads your codebase, decides where it falls under the EU AI Act, and answers every obligation with a file, a line and a hash.
...
And almost every conformity dossier answers them with a document the company wrote about itself — Article 43(1) sends some Annex III biometrics to a notified body, and Annex I products carry sectoral third-party assessment, but even there the assessor reads the documentation rather than the repository. That is not a criticism of anyone's diligence — it is the only thing the process allows. The person filling in the dossier cannot read the repository, and the people who can read the repository are never asked.
...
*Keyword prefilters.* Each detector carries a lowercase substring check against the whole file before any line is scanned, so most files never reach the line loop. A 206-file repository — this one — scans in ~690 ms.

*The Article 99(6) inversion.* Fines are normally the *higher* of a flat cap and a percentage of turnover — except for SMEs and start-ups, where Article 99(6) caps them at the **lower** of the two. Almost every summary of the Act gets this backwards. Annex implements the actual arithmetic, which is why the EU AI Act line for a 40-person company with €9.8m turnover reads €686,000 rather than a meaningless €35 million. (The headline on that scan is €20,000,000 — the GDPR Article 83(5) ceiling for the same failures, because Article 83 has no inversion. Getting *that* right meant modelling each regime on its own terms instead of picking whichever tier sat second in an array.) It also implements the definition the Article borrows: Recommendation 2003/361/EC needs headcount *and* a financial test, so unknown financials resolve to *not* an SME. The inversion lowers the cap, and guessing in the generous direction hands an operator a ceiling an order of magnitude too low.

**Stack:** Node 22, TypeScript strict, Next.js 16 / React 19 server components, Tailwind 4, SQLite via `better-sqlite3`, `jose` for sessions, Vitest, Playwright. A 50-case benchmark, a unit suite, and an end-to-end walk of the running product from an empty database; CI typechecks, tests, benchmarks, audits the UI for contrast and keyboard order, and then runs Annex on Annex.
