...
**Performance.** Each detector carries a lowercase keyword prefilter checked against the whole file before any line is scanned. Most files never reach the line loop. A 191-file repository — this one — scans in ~450 ms; the 48-case benchmark runs in ~120 ms.
