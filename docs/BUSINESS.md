...
**Cost of goods sold is close to nothing, and that is a design consequence, not luck.** The engine is deterministic and offline: classification and control evaluation never call a model. A 191-file repository — this one — evaluates 52 obligations in about 450 ms of a single core. A customer scanning 40 systems on every push, at 200 pushes a system a month, spends about **an hour of CPU a month**. The only variable cost that scales with usage is storage of reports and ledgers, which are tens of kilobytes each.
