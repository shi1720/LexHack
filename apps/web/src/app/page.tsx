...
import { HERO_COMMAND, HERO_GAP_COUNT, HERO_OUTPUT } from './hero-capture';
    ...
    <>
      ...
      <main id="main">
        ...
        <section className="mx-auto max-w-6xl px-5 pb-16 pt-14 sm:pt-20">
          ...
          {/*
            Literal CLI output, generated into `hero-capture.ts` by
            `npm run artefacts` and rendered here.

            It used to be hand-typeset JSX under a comment insisting it was
            verbatim, and it had drifted from the real run in six places —
            the ledger fingerprint, both scores, the exposure headline, the
            timing, and the shape of the gap lines. A product whose argument
            is that documents people write about themselves are worthless
            cannot open with a mock-up of its own output. The colouring below
            is applied to whole lines; not one character of the text is
            written by hand.
          */}
          <div className="card mt-10 overflow-hidden" style={{ background: 'var(--surface)' }}>
            <div className="flex flex-wrap items-center gap-2 border-b px-4 py-2.5" style={{ borderColor: 'var(--line)', background: 'var(--sunken)' }}>
              ...
              <span className="code" style={{ color: 'var(--ink)' }}>{HERO_COMMAND}</span>
            </div>
          ...
          <p className="mt-3" style={{ fontSize: 12.5, color: 'var(--ink-faint)' }}>
            The first {HERO_OUTPUT.split('\n').length} lines of the real run, generated by <span className="code">npm run artefacts</span> and
            not edited by hand; {HERO_GAP_COUNT} gaps follow, each with its citation and its remediation. The EU AI Act figure
            applies the Article 99(6) SME inversion to a €9.8m turnover — the €20m headline is the GDPR Article 83(5)
            ceiling for the same failures, which Article 83 does not invert. The ledger fingerprint reproduces on any
            machine, at any time, from the same commit.
          </p>
        </section>
...
/**
 * Colour a line of captured CLI output.
 *
 * Whole lines only, keyed off what the CLI itself prints. Nothing here may
 * change the text — the point of the capture is that it is the text.
 */
function TerminalLine({ line }: { line: string }) {
  if (line.startsWith(' PROHIBITED')) {
    const rest = line.slice(' PROHIBITED '.length);
    return (
      <>
        {'  '}
        <span style={{ background: 'var(--crimson)', color: 'var(--paper)', fontWeight: 700, padding: '1px 7px', borderRadius: 3 }}>
          {' PROHIBITED '}
        </span>
        {rest}
        {'\n'}
  ...
  const colour = /^\s{4}[✖]/.test(line)
    ...
    : /^\s{4}[▲]/.test(line)
      ...
      : /^\s{2}[✖]/.test(line)
        ...
        : /^\s{2}[▲]/.test(line)
          ? 'var(--amber)'
          : /^\s{4}(src|lib|docs|evals)\//.test(line)
            ? 'var(--ink-soft)'
            : /^\s{4}(EU AI Act|GDPR|NYC|Colorado|NIST)/.test(line)
              ? 'var(--navy)'
              : /^(Classification|Gaps|─)/.test(line)
                ? 'var(--ink-faint)'
                : undefined;
  return <span style={colour ? { color: colour } : undefined}>{line + '\n'}</span>;
}
